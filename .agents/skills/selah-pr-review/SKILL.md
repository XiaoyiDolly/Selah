...
3. Identify at most five consequential, technically supported findings and up to three evidence-backed strengths. Read [review-input.md](references/review-input.md), then create the structured JSON input containing only relevant diff hunks. Pass it to Selah either as an owner-only temporary file (`--input <json>`) or by piping it on stdin.
4. Run `npm run --silent selah -- prepare --pr <url> --input <json>` (or pipe the JSON: `<json-producer> | npm run --silent selah -- prepare --pr <url>`), then remove any temporary input. Do not call Gloo or YouVersion directly.
